...
is.output=F
is.graph=T
varlist=c('Monster','Price','TV_worb','TV_cartoon','TV_flanker','FB_likes','Twitter','Youtube','WTD')
...
iter=10 
level_type=2 # For Order -- dlmModPoly
var_coef="est" 
var_level='est'
var_slope='est'
  ...
  result=foreach(i = 1,.combine='comb', .multicombine=T,.packages=c("dlm"),
                 .init=list(list(),list(),list(),list(),list(),list(),list())) %dopar% { 
                   ...
                   build=function(p){
                     ...
                     if (length(varlist) !=0){
                       if(var_coef=="est"){
                         reg=dlmModReg(x[[i]],addInt=F,dV=0,dW =p[n+1:length(x[[1]])+no.par+1])
                       }else{
                   ...
                   mle=dlmMLE(y[[i]],rep(0,length(x[[1]])+no.par+1),build,method=optim.method,lower=optim.lb,upper=optim.ub)
